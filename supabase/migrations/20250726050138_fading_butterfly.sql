/*
  # Complete DOCMe Database Schema with Razorpay Integration

  1. New Tables
    - `users` - User accounts and authentication
    - `subscriptions` - User subscription plans with Razorpay
...
CREATE TABLE IF NOT EXISTS subscriptions (
  ...
  status text NOT NULL CHECK (status IN ('active', 'cancelled', 'past_due', 'unpaid', 'created', 'authenticated', 'halted')),
...
DECLARE
  invoice_num text;
BEGIN
  SELECT 'INV-' || TO_CHAR(now(), 'YYYY') || '-' || LPAD(NEXTVAL('invoice_sequence')::text, 6, '0') INTO invoice_num;
  RETURN invoice_num;
END;
$$ LANGUAGE plpgsql;
